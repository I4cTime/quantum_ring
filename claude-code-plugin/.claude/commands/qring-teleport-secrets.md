...
argument-hint: "[pack|unpack]"
allowed-tools: mcp__q-ring__teleport_pack, mcp__q-ring__teleport_unpack, mcp__q-ring__list_secrets
---

# /qring-teleport-secrets

Securely transfer secrets between machines using AES-256-GCM encrypted bundles.

If `$1` is `pack` or `unpack`, skip the prompt and proceed directly. Otherwise ask the user which mode they want.
